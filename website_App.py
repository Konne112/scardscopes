...
class Artifact(db.Model):
    ...
    qr_detail_path = db.Column(db.String(200))
...
# ---------------------------------------------------------
# Login-QR-Code EINMAL erzeugen
# ---------------------------------------------------------
login_url = "https://scardscopes.onrender.com/login"
qr_login = qrcode.make(login_url)

qr_dir = os.path.join("static", "qrcodes")
os.makedirs(qr_dir, exist_ok=True)

qr_login_filename = "LOGIN_QR.png"
qr_login_path = os.path.join(qr_dir, qr_login_filename)
qr_login.save(qr_login_path)

LOGIN_QR_PATH = f"static/qrcodes/{qr_login_filename}"
...
def save():
    ...
    custom_era = request.form.get("customEra", "").strip()
    ...
    # ---------------------------------------------------------
    # Eigene Epoche überschreiben
    # ---------------------------------------------------------
    if era == "custom" and custom_era:
        era = custom_era
    ...
    # QR-Code – Detailseite
    # ---------------------------------------------------------
    detail_url = f"https://scardscopes.onrender.com/artifact/{artifact.id}"
    qr_detail = qrcode.make(detail_url)

    qr_detail_filename = f"{inventory_number}_DETAIL.png"
    qr_detail_path = os.path.join(qr_dir, qr_detail_filename)
    qr_detail.save(qr_detail_path)

    artifact.qr_detail_path = f"static/qrcodes/{qr_detail_filename}"
    db.session.commit()
...
def delete(id):
    ...
    if artifact.qr_detail_path:
        try:
            os.remove(artifact.qr_detail_path)
        except:
            pass
